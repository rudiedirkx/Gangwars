...
$engage	= isset($_POST['engage']) ? $_POST['engage'] : false;
...
<head>
<title>Create City - Stage <?php echo $step; ?></title>
<style>
...
switch ( $step )
{
    case 1:
		...
		<input type=hidden name=engage value=1><input type=hidden name=step value=2>
    ...
    case 3:
		...
		$szQuery = "TRUNCATE ".$TABLE['sectors'].";";
		echo $szQuery . " - ";
		$bQueryExec = mysql_query($szQuery) or die(mysql_error());
		echo $bQueryExec ? $success : $failure;
		echo "<br>";

		$k=0;
		$szQuery = "INSERT INTO ".$TABLE['sectors']." (map_id, map_location, name) VALUES ";
		for ( $i=1; $i<=$MAX_SECTORS; $i++ )
		{
			$szQuery .= "(NULL, '$i', 'CityPart $i'),\n";
		...
		$szQuery = substr($szQuery, 0, -2) . ';';
		echo '<pre>' . $szQuery . '</pre>';
		var_dump( mysql_query($szQuery) );

		?>
		<br>
		<form method=post action=create_city.php>
		<input type=hidden name=p_P value='<?php echo $_POST['p_P']; ?>'>
		<input type=hidden name=p_R value='<?php echo $_POST['p_R']; ?>'>
		<input type=hidden name=p_A value='<?php echo $_POST['p_A']; ?>'>
		<input type=hidden name=p_S value='<?php echo $_POST['p_S']; ?>'>
		<input type=hidden name=p_T value='<?php echo $_POST['p_T']; ?>'>
		<input type=hidden name=p_H value='<?php echo $_POST['p_H']; ?>'>
		<input type=hidden name=p_W value='<?php echo $_POST['p_W']; ?>'>
		<input type=hidden name=p_M value='<?php echo $_POST['p_M']; ?>'>
